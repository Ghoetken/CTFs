...
void add_precoded_vault() {
    if (g_vaults.id >= MAX_VAULTS) {
        printf("Vault limit reached. Cannot add precoded vault.\n");
        return;
    }

    Vault *vault = &g_vaults.vaults[g_vaults.id - 1];

    const char *flag = "flag{1_tH0uGhT_Th15_v4UlT_w4s_53curE!}";
    size_t flag_len = strlen(flag);
    vault->data = malloc(0x30);
    if (!vault->data) {
        printf("Failed to allocate memory for precoded vault.\n");
        return;
    }

    memcpy(vault->data, flag, flag_len);
    vault->size = 0x30;

    memset(vault->password, 0, MAX_PASSWORD);  // clear buffer
    memcpy(vault->password, "-!-kaCh0w-!-", strlen("-!-kaCh0w-!-"));
    g_vaults.id++;
}
